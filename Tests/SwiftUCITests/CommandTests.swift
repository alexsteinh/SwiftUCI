...
final class CommandTests: XCTestCase {
    func testIdCommand() {
        ...
        XCTAssertEqual(Command.id(.name("AS Chess Engine")), Command(string: "id name AS Chess Engine"))
        XCTAssertNil(Command(string: "id bogus"))
    }
    ...
    func testBestmoveCommand() {
        XCTAssertEqual(Command.bestmove(.init(from: "e2", to: "e4"), ponder: nil), Command(string: "bestmove e2e4"))
        XCTAssertEqual(Command.bestmove(.init(from: "e2", to: "e4"), ponder: .init(from: "d2", to: "d4")), Command(string: "bestmove e2e4 ponder d2d4"))
        XCTAssertNil(Command(string: "bestmove e2e4 bogus d2d4"))
    }
    
    func testCopyrightCommands() {
    ...
    func testOptionCommand() {
        ...
        XCTAssertNil(Command(string: "option name Nullmove type chec default true"))
    }
}
